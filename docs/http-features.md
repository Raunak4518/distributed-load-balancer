...
- Response timing has two separate bounds: `forward_timeout_ms` (default 5s) covers dialing and waiting for the backend's response headers, and `response_body_idle_timeout_ms` (default 60s) bounds every gap between chunks of the response body as it is streamed to the client. A backend that stalls mid-body is cut off, the client's response is aborted rather than presented as complete, and `lb_request_timeouts_total{phase="upstream_body"}` increments. The default is generous so long-lived streams such as server-sent events keep working; lower it for APIs whose responses should never pause for long.
