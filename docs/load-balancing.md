...
- **Different backend:** the retry loop does not maintain its own exclusion set. On a failed attempt, the circuit breaker's `record_failure()` result is propagated into the pool's `circuit_open` flag immediately, before the next `pick()` runs, so a backend that trips its breaker on the first failure (e.g. `failure_threshold = 1`) is excluded from the retry's selection. With a higher failure threshold, a single failure may not be enough to open the circuit, and the retry can land on the same backend again — selection is a fresh `pick()` against the (possibly still-current) eligible set, not a "try a different one" instruction.
