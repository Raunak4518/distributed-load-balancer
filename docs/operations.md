...
State that survives a reload of the *same* listener: each backend's manual drain (set via the admin API, see below), its last health-check verdict, and its circuit breaker's open/cooldown state. A backend that was failing before the reload stays out of rotation after it, and a backend newly added by the reload waits for its first successful probe. A `dns_discovery` listener whose discovery settings are unchanged keeps its resolved backends. Outlier ejections are the exception and are re-derived from fresh traffic; see [`health-checking.md`](health-checking.md#config-reload).
...
| `GET /backends` | JSON: every listener, each with its pools (`default`, `route:<n> (...)`, `canary:<n> (<percent>%)`), each backend's `id`, `address`, `active_healthy`, `circuit_open`, `manually_drained`, `outlier_ejected`, `awaiting_first_probe`, `eligible`, and `active_conns`. |
...
| Cluster nodes report peer-sync errors / counts look wrong across nodes | Peer HMAC authentication failing (mismatched shared secret), or a peer mTLS handshake failing | `warn`-level log lines `rejected an unauthenticated peer message` or `peer tls handshake failed`, naming the peer, and `lb_cluster_auth_failures_total` / `lb_cluster_peer_sync_total{outcome="auth_failed"}`; see [`cluster-coordination.md`](cluster-coordination.md) |
